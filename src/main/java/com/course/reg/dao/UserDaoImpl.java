...
public class UserDaoImpl implements UserDao {
	...
	public String enrollTopicName(String loginusername, String topicName) {
		...
		RowMapper<Course> rowMapper = new BeanPropertyRowMapper<Course>(Course.class);
		List<Course> courselist = jdbcTemplate.query(query, rowMapper, loginusername, topicName);
	...
	public List<Course> getAllEnrolledCoursesByLoginUsername(String loginUsername) {
		...
		RowMapper<Course> rowMapper = new BeanPropertyRowMapper<Course>(Course.class);
		List<Course> courselist = jdbcTemplate.query(query, rowMapper, loginUsername);
		
		 String inParams = String.join(",", courselist.stream().map(course -> "'"+course.getTopicName()+"'").collect(Collectors.toList()));
		 List<Course> coursesList = jdbcTemplate.query(String.format("SELECT * FROM courses_details WHERE topicName IN (%s)",inParams), rowMapper);
	...
	public String saveCourse(Course course) {
		...
		RowMapper<Course> rowMapper = new BeanPropertyRowMapper<Course>(Course.class);
		List<Course> courselist = jdbcTemplate.query(query, rowMapper,course.getTopicName());
	...
	public Course getCourseByTopicName(String topicName) {
		String query = "SELECT * FROM courses_details WHERE topicName = ?";
		RowMapper<Course> rowMapper = new BeanPropertyRowMapper<Course>(Course.class);
		List<Course> course =  jdbcTemplate.query(query, rowMapper,topicName);
		return course.get(0);
	}
