...
public class UsersMvcController {
	...
	public String saveUserRegistration(@Valid @ModelAttribute UserRegistration userRegistration, BindingResult result,
			ModelMap map) {
		if (result.hasErrors()) {
			return "userRegistrationForm";
		}
	...
	public String getUserProfile(ModelMap map) {
		UserProfile userRegistration = userService.findUserProfileByLoginUsername(securityUtil.getLoginUsername());
	...
	public String updateUserProfileFormPage(@PathVariable String loginUsername, ModelMap map) {
		UserProfile user = userService.findUserProfileByLoginUsername(loginUsername);
		map.addAttribute("userProfile", user);
		return "updateUserProfileForm";
	}

	@PostMapping(value = "/profile/update")
	public String updateUserProfile(@Valid @ModelAttribute UserProfile userProfile, BindingResult br) {
		if (br.hasErrors()) {
			return "updateUserProfileForm";
		}
		userService.updateUserProfileByLoginUsername(securityUtil.getLoginUsername(), userProfile);
		return "redirect:/view/profile";
	}

	@GetMapping(value = "/setProfilePic")
	public String getProfilePicForm() {
	...
	public String enrollTopicName(@PathVariable(name = "topicName", required = true) String topicName, ModelMap map) {
		String res = userService.enrollTopicName(securityUtil.getLoginUsername(), topicName);
		map.put("enroll", res);
	...
	public String searchTopic(@RequestParam("topic") String topic, ModelMap map) {
		List<Course> courseList = userService.searchAllTopicsByTopicName(topic);
		map.put("courseList", courseList);
		return "viewAllCourses";
	}
	...
	@GetMapping(value = "/search/topicName")
	public String searchTopicByToicName(@RequestParam("topicName") String topicName, ModelMap map) {
		List<Course> courseListForUser = userService.searchAllTopicsByTopicName(topicName);
		map.put("enrolledCourseList", courseListForUser);
		return "viewAllEnrolledCoursesForUser";
	}
