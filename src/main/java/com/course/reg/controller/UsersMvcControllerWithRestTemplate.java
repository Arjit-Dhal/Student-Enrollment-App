...
public class UsersMvcControllerWithRestTemplate {
	...
	public String saveUserRegistration(@ModelAttribute UserRegistration user, ModelMap map) throws URISyntaxException {
		
		HttpHeaders headers = new HttpHeaders();
		headers.set("Accept", "application/json");
	    HttpEntity<UserRegistration> request = new HttpEntity<>(user, headers);
	    //OR
	...
	public String getUserProfile(ModelMap map) {
		final ResponseEntity<UserRegistration> responseEntity = restTemplate.getForEntity("http://localhost:8081/api/rest/profile", UserRegistration.class);
		UserRegistration user = responseEntity.getBody();
		map.put("res", user);
		return "consumer/userProfile";
	}
	...
	public String userProfileUpdate(@ModelAttribute UserRegistration user, ModelMap map, BindingResult br) {
	...
	private HttpEntity<UserRegistration> getEntityWithHeaders(UserRegistration user) {
		HttpHeaders headers = new HttpHeaders();
		headers.setAccept(Arrays.asList(MediaType.APPLICATION_JSON));
		HttpEntity<UserRegistration> entity = new HttpEntity<UserRegistration>(user, headers);
		return entity;
	}
}
